...
說明：
  - 每張資料表輸出一支 CSV，檔名前綴數字即匯入順序（01_、02_...）。
  - CSV 含標頭列，欄位名稱對齊資料表實際欄名（eId / catId / pId / uId / custId / oId / sId ...）。
  - 含外鍵的欄位直接使用對應資料表的鍵欄名（如 catId、pId、eId）。
"""
import csv
import os
import random
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # ---- 員工 employees ----
        # 欄位：eId, password, username, name, cellphone, address, resigned_date
        emp_rows = []
        admin_pw = make_password("admin12345")
        staff_pw = make_password("staff12345")
        emp_rows.append([1, admin_pw, "admin", "管理員", "0900000000", "", None])
        roles = ["業務", "業務", "主管", "員工", "員工"]
        for i, role in enumerate(roles, 1):
            eid = i + 1
            name = f"{random.choice(CUSTOMER_SURNAMES)}{role}{i}"
            cell = f"09{random.randint(10000000, 99999999)}"
            emp_rows.append([eid, staff_pw, f"staff{i}", name, cell, "", None])
        emp_ids = [r[0] for r in emp_rows]
        self._write(out_dir, "employees",
                    ["eId", "password", "username", "name",
                     "cellphone", "address", "resigned_date"], emp_rows)

        # ---- 單位 units ----
        # 欄位：uId, name
        unit_id = {name: i for i, name in enumerate(UNITS, 1)}
        unit_rows = [[uid, name] for name, uid in unit_id.items()]
        self._write(out_dir, "units", ["uId", "name"], unit_rows)

        # ---- 分類 categories ----
        # 欄位：catId, name
        cat_id = {name: i for i, name in enumerate(CATEGORIES, 1)}
        cat_rows = [[cid, name] for name, cid in cat_id.items()]
        self._write(out_dir, "categories", ["catId", "name"], cat_rows)

        # ---- 商品 products + 商品單位定價 product_unit ----
        # product 欄位：pId, name, catId, eId, description
        # product_unit 欄位：id, conversion_rate, price, cost, status, pId, uId
        product_rows = []
        pu_rows = []
        product_units = {}     # product_id -> list of (price, cost, conversion_rate)
        ...
        for cat_name, names in PRODUCTS.items():
            for name in names:
                ...
                product_rows.append([pid, name, cat_id[cat_name], 1, ""])
                base_unit_of[pid] = base
                base_cost[pid] = cost
                product_units[pid] = []

                puid += 1
                pu_rows.append([puid, 1, price, cost, "active", pid, base])
                product_units[pid].append((price, cost, 1))
                ...
                if random.random() < 0.5:
                    ...
                    puid += 1
                    pu_rows.append([puid, rate, bprice, bcost, "active", pid, unit_id["箱"]])
                    product_units[pid].append((bprice, bcost, rate))
        product_ids = list(range(1, pid + 1))
        self._write(out_dir, "products",
                    ["pId", "name", "catId", "eId", "description"], product_rows)
        self._write(out_dir, "product_unit",
                    ["id", "conversion_rate", "price", "cost", "status", "pId", "uId"], pu_rows)

        # ---- 客戶 customers ----
        # 欄位：custId, name, cellphone, address, note
        cust_rows = []
        ...
        self._write(out_dir, "customers",
                    ["custId", "name", "cellphone", "address", "note"], cust_rows)

        # ---- 進貨批次 stocks ----
        # 欄位：sId, quantity, quantity_remaining, unit_cost, pId, uId
        stock_rows = []
        sid = 0
        for pid in product_ids:
            for _ in range(random.randint(1, 3)):
                ...
                stock_rows.append([
                    sid, qty_base, qty_base, base_cost[pid], pid, base_unit_of[pid],
                ])
        self._write(out_dir, "stocks",
                    ["sId", "quantity", "quantity_remaining", "unit_cost",
                     "pId", "uId"], stock_rows)

        # ---- 訂單 orders + 明細 order_record ----
        # order 欄位：oId, order_date, status, custId, eId
        # record 欄位：id, price, cost, quantity, discount, conversion_rate, oId, pId
        ...
        for i in range(opts["orders"]):
            ...
            order_rows.append([oid, ordered_str, status,
                               random.choice(customer_ids), random.choice(emp_ids)])
            for _ in range(random.randint(1, 4)):
                pid = random.choice(product_ids)
                price, cost, rate = random.choice(product_units[pid])
                discount = random.choice([Decimal("0"), Decimal("0.05"),
                                          Decimal("0.10"), Decimal("0.15")])
                rid += 1
                record_rows.append([
                    rid, price, cost, random.randint(1, 5), discount, rate, oid, pid,
                ])
        self._write(out_dir, "orders",
                    ["oId", "order_date", "status", "custId", "eId"], order_rows)
        self._write(out_dir, "order_record",
                    ["id", "price", "cost", "quantity", "discount",
                     "conversion_rate", "oId", "pId"], record_rows)
    ...
    def _seed_to_db(self, opts):
        ...
        admin = Employee.objects.filter(username="admin").first()
        if not admin:
            admin = Employee.objects.create_user(
                username="admin", password="admin12345",
                name="管理員", cellphone="0900000000",
            )
        ...
        # 商品 + 第一筆 ProductUnit + 部分加箱單位
        products = []
        base_unit_of = {}
        for cat_name, names in PRODUCTS.items():
            for name in names:
                base = random.choice([units["個"], units["瓶"], units["罐"], units["盒"]])
                price = Decimal(random.randint(15, 200))
                cost = (price * Decimal("0.6")).quantize(Decimal("0.01"))
                p, created = Product.objects.get_or_create(
                    name=name,
                    defaults={"category": cats[cat_name], "created_by": admin},
                )
                base_unit_of[p.pk] = base
                if created:
                    ProductUnit.objects.create(
                        product=p, unit=base, conversion_rate=1, price=price, cost=cost,
                    )
                    if random.random() < 0.5:
                        rate = random.choice([6, 12, 24])
                        ProductUnit.objects.create(
        ...
        Stock.objects.all().delete()
        for p in products:
            base = base_unit_of[p.pk]
            for _ in range(random.randint(1, 3)):
                pcs_pu = p.product_units.filter(unit=base, status=ProductUnit.Status.ACTIVE).first()
                qty_base = random.randint(50, 300)
                Stock.objects.create(
                    product=p, unit=base,
                    quantity=qty_base, quantity_remaining=qty_base,
                    unit_cost=pcs_pu.cost if pcs_pu else Decimal("10"),
                )
        ...
        for i in range(opts["orders"]):
            ...
            order = Order.objects.create(
                customer=customer, ordered_date=ordered, status=status,
                created_by=random.choice(employees),
            )
            for _ in range(random.randint(1, 4)):
                p = random.choice(products)
                pu = random.choice(list(p.product_units.filter(status=ProductUnit.Status.ACTIVE)))
